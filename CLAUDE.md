...
│   └── agents/
│       ├── base_agent.py              ← BaseAgent (Anthropic + Gemini, temperature=0)
│       ├── test_reader_agent.py       ← Agent 1: intake → structured test cases + DATA blocks
│       ├── bdd_generator_agent.py     ← Agent 2: → feature + steps + locators + DataFactory
│       ├── self_heal_agent.py         ← Agent 3: XPath self-heal via MCP
│       ├── failure_analysis_agent.py  ← Agent 4: classifies failures, produces fix plans
│       ├── approval_gateway.py        ← Orchestrator: routes failures → Agent 3 / 4
│       └── pipeline.py                ← CLI: intake file → all 4 output files
│
├── tests/
│   ├── api/
...
│
├── intake/                       ← plain-text / Excel test cases → feed to pipeline
│   ├── TEMPLATE.txt              ← format guide for writing intake test cases
│   └── {app}_{screen}.txt        ← one file per screen (explicit DATA: values)
│
├── testCases/
│   ├── features/                 ← BDD Gherkin scenarios (generated by Agent 2)
│   │   └── steps/                ← step definitions → Facade only (NOT PageObject)
│   └── api/
...
TRADEME_SEARCH_LOCATORS = {
    ...
    "search": {
        ...
        "button": "",   # //button[normalize-space()='Search']
    },

    # ── filters ───────────────────────────────────────────────
    "filters": {
        "min_price":    "",   # //input[@name='price_min'] or //select[@aria-label='Min price']
...
**Rules:**
- ALL CAPS dict name · nested two-level dict · empty string values — tester fills
- XPath selector priority: `data-testid` > `aria-label` > `placeholder` > `name` > stable text > structural XPath
- **Never CSS class selectors** — no `.class-name`, no `input.something`, no auto-generated names
- Use `scripts/capture_locators.py` to fill values interactively (XPath captured on click)
...
|------|------|-----------|---------|
| 1 | `test_reader_agent.py` | YES | reads intake file → structured test cases + DATA blocks |
| 2 | `bdd_generator_agent.py` | YES | generates feature + steps + locators + DataFactory |
| 3 | `pipeline.py` | YES | end-to-end: intake file → all 4 output files |
| 4 | (direct edit) | NO | adds one method to existing class |
| 5 | `self_heal_agent.py` | NO | scoped XPath fix via MCP |
| 6 | `approval_gateway.py` | NO | post-run orchestrator: routes failures to Agent 3/4 |
| 7 | `failure_analysis_agent.py` | NO | AI root-cause analysis → fix plan file |

**AI output is deterministic:** All agents use `temperature=0`. Same intake file always produces the same output.

**DATA blocks ensure exact values:** Agent 1 extracts concrete values from intake into a `DATA:` section. Agent 2 copies those values verbatim into Gherkin steps — no AI invention of test data.
...
MOCK_MODE=true  pytest tests/api/ -v          # API tests (mock mode)
MOCK_MODE=false pytest tests/api/ -v          # API tests (live)
pytest tests/e2e/ -v --headed                 # E2E tests
pytest tests/ --browser firefox -v            # specific browser
pytest tests/ --reruns 2 -v                   # with retries

# ── Agent pipeline (intake → BDD + steps + locators + DataFactory) ────
...
# ── Locator capture (interactive XPath picker) ────────────────────────
python scripts/capture_locators.py \
    --file locators/trademe_property_search_locators.py \
    --url  "https://www.trademe.co.nz/a/property/residential/sale" \
    --all

# ── Orchestrator (run after failures) ────────────────────────────────
python -m utils.agents.approval_gateway          # manual run
AUTO_HEAL=true pytest tests/ -v                  # auto-fires at session end
